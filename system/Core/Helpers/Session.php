<?php
    ...
    class Session
    {
        ...
        public static function set($key, $value = false)
        {
            ...
            if (is_array($key) && $value === false) {
                foreach ($key as $name => $value) {
                    $_SESSION[getenv("SESSION_PREFIX").$name] = $value;
                }
            } else {
                $_SESSION[getenv("SESSION_PREFIX").$key] = $value;
            }
        }
        ...
        {
            if (isset($_SESSION[getenv("SESSION_PREFIX").$key])) {
                $value = $_SESSION[getenv("SESSION_PREFIX").$key];
                unset($_SESSION[getenv("SESSION_PREFIX").$key]);
                return $value;
            }
            return null;
        }
        ...
        {
            if ($secondkey == true) {
                if (isset($_SESSION[getenv("SESSION_PREFIX").$key][$secondkey])) {
                    return $_SESSION[getenv("SESSION_PREFIX").$key][$secondkey];
                }
            } else {
                if (isset($_SESSION[getenv("SESSION_PREFIX").$key])) {
                    return $_SESSION[getenv("SESSION_PREFIX").$key];
                }
            }
            return null;
        }
        ...
        /**
         * Check if a message on the specified index
         * @param $message
         * @return bool
         */
         ...
         * @param  boolean $prefix - if set to true clear all sessions for current getenv("SESSION_PREFIX")
         *
         */
        public static function destroy($key = '', $prefix = false)
        {
            ...
            if (self::$sessionStarted == true) {
                ...
                if ($key =='' && $prefix == false) {
                    ...
                    /** clear all session for set getenv("SESSION_PREFIX") */
                    foreach ($_SESSION as $key => $value) {
                        if (strpos($key, getenv("SESSION_PREFIX")) === 0) {
                            unset($_SESSION[$key]);
                        }
                    }
                } else {
